...
function renderTree() {
  ...
  const withTr = curTree.causes.filter(c => c.tr);
  const noFmea = curTree.causes.filter(c => !c.fmea);
        ...
        <table>
          ...
          <tbody>${curTree.causes.map((c, i) => `
            <tr>
              ...
              <td class="nowrap">${c.tr ? `<span class="mono">${esc(c.tr)}</span>` : '<span class="cell-empty">—</span>'}</td>
              <td class="nowrap">${c.fmea ? esc(procLabel(c.fmea))
