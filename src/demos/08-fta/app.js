...
const TOPW = 220, GATEW = 215, CAUW = 340, RH = 66, RGAP = 10, COLGAP = 52;
...
function treeSvg() {
  ...
  const gateBoxes = rows.map(r => `
    <g class="node" tabindex="0" role="button" data-gate="${esc(r.g.id)}"
      ...
      <text x="${x1 + 10}" y="${r.cy - 16}" style="font-size:14px;fill:var(--color-primary)">中間事象　${esc(r.g.op)}</text>
      ...
      <text x="${x1 + GATEW - 8}" y="${r.cy + 26}" text-anchor="end"
        style="font-size:14px;fill:var(--color-text-secondary)">${r.causes.length} 件</text>
    </g>`).join('');
  ...
  const cauBoxes = rows.map(r => r.causes.map((c, i) => {
    ...
    return `<g class="node" tabindex="0" role="button" data-cause="${idx}"
      ...
      <text x="${x2 + 12}" y="${cy - RH / 2 + 17}"
        style="font-size:14px;fill:${marked ? 'var(--color-warning)' : 'var(--color-text-secondary)'}">${esc(m ? m.label : c.m)}${marked ? '　◆' : ''}</text>
      ${wrapText(c.label, 21).slice(0, 2).map((t, k) => `<text x="${x2 + 12}"
        y="${cy - RH / 2 + 38 + k * 18}">${esc(t)}</text>`).join('')}
      <text x="${x2 + CAUW - 10}" y="${cy - RH / 2 + 17}" text-anchor="end"
        style="font-size:14px;fill:var(--color-text-secondary)">${hit ? '関連記録あり' : '実績なし'}</text>
    </g>`;
  }).join('')).join('');
